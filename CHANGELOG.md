...
- **'라인삭제' 툴 제거** — 마크업 선택·삭제 모드가 "경계 라인을 지우는 기능"으로
  오해를 일으킴. 툴바에서 제거하고, 요청 삭제는 수정요청 카드의 [요청삭제] 버튼으로
  이동. 관련 죽은 코드 정리: MapView eraseMode/DragBox 다중선택, DrawHint 삭제모드
  분기, 일괄 삭제 모달.
- **카드 머리 재배치** — 상태 텍스트(처리대기/반영됨/반려)를 종류 배지 바로 옆으로,
  오른쪽 자리에는 [요청삭제] 버튼 (반영된 요청은 삭제 불가 → 버튼 숨김).
- **삭제 확인 모달** — [요청삭제]를 눌러도 바로 지우지 않고 "정말 삭제하시겠습니까?
  삭제하면 복구할 수 없습니다" 확인을 거친다. 버튼 클릭 시 해당 마크업을 지도에서
  노란 강조로 표시해 무엇이 지워지는지 보여준다.
- **기입력 행정리명·부호 라벨** — 경계 데이터에 이미 들어있는 ri_nm/ri_cd 를 폴리곤
  안에 갈색(#92400e) 라벨로 표시 — 속성등록 요청 라벨(파랑)과 색으로 구분.
- **라벨 겹침 처리** — 행정리·수정요청 레이어에 OL declutter 적용(겹치는 텍스트 자동
  숨김, 수정요청 라벨 우선). 행정리 라벨은 줌이 멀면(해상도 20m/px 초과) 표시 안 함,
  작은 폴리곤에 안 들어가는 라벨도 숨김(overflow:false).
- 검증: 웹 `tsc -b` + vite build OK.
