...
def _build_markup_layer(features, kind, attr_label_field='ri_nm'):
    """kind 별 feature 부분집합을 별도 GeoJSON 임시파일로 저장 → 레이어.

    feature.properties.attrs 는 dict 라서 GDAL OGR 가 못 읽음 → attrs 의 키를
    개별 컬럼(ri_cd, ri_nm 등)으로 평탄화한 뒤 ogr 로 로드.
    """
    import json
    import tempfile
    from qgis.core import (QgsVectorLayer, QgsProject, QgsLineSymbol,
                           QgsMarkerSymbol, QgsSingleSymbolRenderer,
                           QgsPalLayerSettings, QgsTextFormat,
                           QgsVectorLayerSimpleLabeling)
    from qgis.PyQt.QtGui import QColor, QFont
    name, color, geom_kind, dashed = _MARKUP_KIND_STYLE[kind]

    flat_feats = []
    for f in features:
        props = dict(f.get('properties') or {})
        attrs = props.pop('attrs', None) or {}
        if isinstance(attrs, dict):
            for k, v in attrs.items():
                props.setdefault(k, v)
        flat_feats.append({**f, 'properties': props})

    if not flat_feats:
        _remove_by_name(name)
        return None, 0

    fc = {'type': 'FeatureCollection', 'features': flat_feats}
    tf = tempfile.NamedTemporaryFile(
        suffix='.geojson', delete=False, mode='w', encoding='utf-8')
    json.dump(fc, tf, ensure_ascii=False)
    tf.close()
    lyr = QgsVectorLayer(tf.name, name, 'ogr')
    if not lyr.isValid():
        return None, 0
    ...
    # 라벨 — attr 마크업은 ri_nm, 다른 kind 도 같은 필드 있으면 표시
    if attr_label_field and any(
            attr_label_field in (f.get('properties') or {}) for f in flat_feats):
        pal = QgsPalLayerSettings()
        pal.fieldName = attr_label_field
        pal.enabled = True
        tf_fmt = QgsTextFormat()
        tf_fmt.setFont(QFont('Noto Sans CJK KR', 9))
        tf_fmt.setSize(9)
        tf_fmt.setColor(QColor('#222'))
        pal.setFormat(tf_fmt)
    ...
    return lyr, len(flat_feats)


def load_markup_layer(geojson_dict, layer_name=None):
    """서버 마크업 FeatureCollection 을 kind 별 readOnly 레이어 3개로 분리 로드.

    feature.properties.kind 가 'add'|'delete'|'attr' 중 하나여야 함. kind 값이
    없거나 알 수 없는 경우 'add' 로 폴백 (구버전 GeoJSON 호환).
    layer_name 은 하위 호환용으로 무시 (kind 별 이름 자동).
    Returns: dict {'add': n_add, 'delete': n_del, 'attr': n_attr, 'total': n}
             또는 단순 (layer, total) — 호출자 호환을 위해 tuple 으로.
    """
    feats = (geojson_dict or {}).get('features') or []
    by_kind = {'add': [], 'delete': [], 'attr': []}
    for f in feats:
        k = ((f.get('properties') or {}).get('kind') or 'add').lower()
        if k not in by_kind:
            k = 'add'
        by_kind[k].append(f)
    counts = {}
    ...
    for kind, items in by_kind.items():
        lyr, n = _build_markup_layer(items, kind)
        counts[kind] = n
        if lyr is not None:
            last_layer = lyr
    counts['total'] = sum(counts.get(k, 0) for k in ('add', 'delete', 'attr'))
    return last_layer, counts
